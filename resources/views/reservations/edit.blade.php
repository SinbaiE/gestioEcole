    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Modifier Réservation {{ $reservation->reservation_number }}
            </h2>
            <a href="{{ route('reservations.show', $reservation) }}" class="inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700">
                Retour
            </a>
        </div>
    </x-slot>
            ...
            <form method="POST" action="{{ route('reservations.update', $reservation) }}" class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="room_type_id" id="room_type_id" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                ...
                                @foreach($roomTypes as $roomType)
                                    <option value="{{ $roomType->id }}" data-price="{{ $roomType->base_price }}" {{ old('room_type_id', $reservation->room_type_id) == $roomType->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="number" name="adults" id="adults" value="{{ old('adults', $reservation->adults) }}" min="1" max="10" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            @error('adults')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="children" id="children" value="{{ old('children', $reservation->children) }}" min="0" max="10" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            @error('children')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="date" name="check_in_date" id="check_in_date" value="{{ old('check_in_date', $reservation->check_in_date->format('Y-m-d')) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            @error('check_in_date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="check_out_date" id="check_out_date" value="{{ old('check_out_date', $reservation->check_out_date->format('Y-m-d')) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <div>
                            ...
                            <input type="number" name="room_rate" id="room_rate" value="{{ old('room_rate', $reservation->room_rate) }}" min="0" step="1000" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            @error('room_rate')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label for="status" class="block text-sm font-medium text-gray-700">Statut de la réservation</label>
                            <select name="status" id="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                ...
                                <option value="checked_in" {{ old('status', $reservation->status) == 'checked_in' ? 'selected' : '' }}>Enregistrée</option>
                                <option value="checked_out" {{ old('status', $reservation->status) == 'checked_out' ? 'selected' : '' }}>Libérée</option>
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <div class="flex justify-between items-center">
                                ...
                                <span id="nights-display" class="text-sm text-gray-900">{{ $reservation->nights }}</span>
                            </div>
                            <div class="flex justify-between items-center mt-2">
                                <span class="text-lg font-medium text-gray-900">Total estimé:</span>
                                <span id="total-display" class="text-lg font-bold text-blue-600">{{ number_format($reservation->total_amount, 0, ',', ' ') }} FCFA</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6">
                    ...
                    <div>
                        ...
                        <textarea name="special_requests" id="special_requests" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Lit bébé, vue mer, étage élevé...">{{ old('special_requests', $reservation->special_requests) }}</textarea>
                    ...
                    <div class="mt-4">
                        ...
                        <textarea name="notes" id="notes" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Notes pour le personnel...">{{ old('notes', $reservation->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    <a href="{{ route('reservations.show', $reservation) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                        Annuler
                    </a>
