<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm rounded-lg">
            <form method="POST" action="{{ route('rooms.store') }}" class="p-6" enctype="multipart/form-data">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="room_number" id="room_number" value="{{ old('room_number') }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Ex: 101, A12, Suite-1">
                        @error('room_number')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="floor" id="floor" value="{{ old('floor') }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Ex: 1, RDC, Mezzanine">
                        @error('floor')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <option value="cleaning" {{ old('status') == 'cleaning' ? 'selected' : '' }}>Nettoyage</option>
                            <option value="out_of_order" {{ old('status') == 'out_of_order' ? 'selected' : '' }}>Hors service</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="housekeeping_status" id="housekeeping_status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            <option value="clean" {{ old('housekeeping_status') == 'clean' ? 'selected' : '' }}>Propre</option>
                            <option value="dirty" {{ old('housekeeping_status') == 'dirty' ? 'selected' : '' }}>Sale</option>
                            <option value="inspected" {{ old('housekeeping_status') == 'inspected' ? 'selected' : '' }}>Inspectée</option>
                            <option value="maintenance" {{ old('housekeeping_status') == 'maintenance' ? 'selected' : '' }}>Maintenance</option>
                        </select>
                    ...
                    <div>
                        ...
                        <textarea name="notes" id="notes" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Notes sur la chambre, équipements spéciaux, etc.">{{ old('notes') }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    <a href="{{ route('rooms.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                        Annuler
                    </a>
