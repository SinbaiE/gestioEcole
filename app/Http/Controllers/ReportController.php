...
class ReportController extends Controller
{
    ...
    public function checkinout(Request $request): View
    {
        $hotelId = auth()->user()->hotel_id;
        $startDate = $request->get('start_date', now()->startOfMonth());
        $endDate = $request->get('end_date', now()->endOfMonth());

        $checkins = Reservation::where('hotel_id', $hotelId)
        ...
        $checkouts = Reservation::where('hotel_id', $hotelId)
        ...
        $checkinData = $dates->map(function ($date) use ($checkins) {
            return $checkins->get($date, 0);
        });

        $checkoutData = $dates->map(function ($date) use ($checkouts) {
            return $checkouts->get($date, 0);
        });

        $avgStay = Reservation::where('hotel_id', $hotelId)
        ...
        return view('reports.checkinout', compact('data', 'startDate', 'endDate'));
    }
}
