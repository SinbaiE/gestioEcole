            ...
            <form method="POST" action="{{ route('services.store') }}" class="p-6">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Ex: Massage relaxant, Transfert aéroport">
                        @error('name')
                    ...
                    <div>
                        ...
                        <select name="category" id="category" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <option value="spa" {{ old('category') == 'spa' ? 'selected' : '' }}>Spa & Bien-être</option>
                            ...
                            <option value="laundry" {{ old('category') == 'laundry' ? 'selected' : '' }}>Blanchisserie</option>
                            <option value="transport" {{ old('category') == 'transport' ? 'selected' : '' }}>Transport</option>
                            <option value="business_center" {{ old('category') == 'business_center' ? 'selected' : '' }}>Centre d'affaires</option>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="price" id="price" value="{{ old('price') }}" min="0" step="500" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            @error('price')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="pricing_type" id="pricing_type" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                ...
                                <option value="per_hour" {{ old('pricing_type') == 'per_hour' ? 'selected' : '' }}>Par heure</option>
                                <option value="per_day" {{ old('pricing_type') == 'per_day' ? 'selected' : '' }}>Par jour</option>
                                <option value="per_person" {{ old('pricing_type') == 'per_person' ? 'selected' : '' }}>Par personne</option>
                            </select>
                            @error('pricing_type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="max_capacity" id="max_capacity" value="{{ old('max_capacity') }}" min="1" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Laisser vide si illimitée">
                        @error('max_capacity')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    <a href="{{ route('services.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                        Annuler
                    </a>
